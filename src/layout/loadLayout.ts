    ...
    case 'keyValueList':
      return {
        ...
        items: needArray(r.items, path + '.items').map((it, i) => {
          ...
          return { key: needString(ir.key, `${path}.items[${i}].key`), value: needString(ir.value, `${path}.items[${i}].value`) };
        }),
      };
    case 'list':
      return {
...
function toSubSection(v: unknown, path: string): PanelSubSection {
  ...
  return {
    ...
    minHeight: r.minHeight === undefined
      ? undefined
    ...
    components: needArray(r.components, path + '.components').map((c, i) => toComponent(c, `${path}.components[${i}]`)),
  };
}
...
function toSection(v: unknown, path: string): PanelSection {
  ...
  return {
    id: needId(r.id, path + '.id'),
    label: needString(r.label, path + '.label'),
    subSections: needArray(r.subSections, path + '.subSections').map((s, i) => toSubSection(s, `${path}.subSections[${i}]`)),
  };
}
