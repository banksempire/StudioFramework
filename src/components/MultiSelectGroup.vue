...
function itemClasses(i: number): Record<string, boolean> {
  ...
  return {
    'sf-ms-item--on': on,
    'sf-ms-item--start': on && !prevOn,
    'sf-ms-item--end': on && !nextOn,
    'sf-ms-item--cont': on && nextOn,
    'sf-ms-item--wrapclose': on && nextOn && rowBreakBefore.value.has(i + 1),
    'sf-ms-item--wrapopen': on && prevOn && rowBreakBefore.value.has(i),
  };
}
...
<template>
  <div class="sf-ms" role="group">
    <div ref="trackEl" class="sf-ms-track">
      <button
